...
export function SurveyPage() {
  ...
  return <div className="page-stack"><PageHeader eyebrow="SURVEY DELIVERY" title={t("surveyTitle")} description={t("surveyDesc")} actions={<EventPicker module="survey" />} />
    <section className="module-hero survey"><span><ClipboardList size={25} /></span><div><small>{t("surveyAfterOnly")}</small><h2>{localize(event.title, language)}</h2><a href={event.survey.url} target="_blank" rel="noreferrer">{event.survey.url}</a></div><LoadingButton className="primary-button" loading={busy} disabled={!eligibility.ok || !people.length} onClick={() => send(people.map((person) => person.id))}><Send size={15} />{t("sendAll")}</LoadingButton></section>
    <div className={eligibility.ok ? "survey-lock-notice survey-ready-notice" : "survey-lock-notice"}>{eligibility.ok ? <CheckCircle2 size={17} /> : <LockKeyhole size={17} />}<strong>{eligibility.ok ? t("surveyReady") : t("surveyLocked")}</strong>{eligibility.unlockAt && !eligibility.ok && <span>{t("surveyUnlockAt")}：{formatDate(eligibility.unlockAt, language)}</span>}</div>
    <section className="delivery-panel"><div className="section-heading"><div><span>ATTENDEES</span><h2>{t("surveyRecipients")} · {people.length}</h2></div></div><div className="delivery-list">{people.length ? people.map((person) => <article key={person.id}><span className="person-avatar">{localize(person.name, language).slice(0, 1)}</span><div><strong>{localize(person.name, language)}</strong><small>{person.id} · {person.email}</small></div><DeliveryStatus record={records[person.id]} t={t} language={language} /></article>) : <EmptyState icon={CalendarClock} title={t("noSurveyRecipients")} />}</div></section>
  </div>;
}
