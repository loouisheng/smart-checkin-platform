...
function EventSelect({ module, includeHistory = false }) {
  const { events, activeEventId, selectActiveEvent, language, t } = useApp();
  const available = events.filter((event) => includeHistory
    ? ["completed", "cancelled"].includes(event.status)
    : event.status !== "cancelled" && (!module || event.modules?.[module]));
  const selectedValue = available.some((event) => event.id === activeEventId) ? activeEventId : "";
  return <label className="event-context-select"><span>{includeHistory ? t("selectHistory") : t("browseEvents")}</span><select value={selectedValue} onChange={(event) => selectActiveEvent(event.target.value)}>
    <option value="" disabled>{t("selectEventPrompt")}</option>
    {available.map((event) => <option value={event.id} key={event.id}>{localize(event.title, language)} · {event.date}</option>)}
  </select></label>;
}
...
export function ReportsPage() {
  ...
  if (!event) return <div className="page-stack reports-page"><PageHeader eyebrow="LIVE OPERATIONS" title={t("reportsTitle")} description={t("reportsDesc")} actions={<EventSelect />} /><EmptyState icon={Users} title={t("selectEventPrompt")} /></div>;
  const kpis = calculateKpis(people, records, event);
  return <div className="page-stack reports-page"><PageHeader eyebrow="LIVE OPERATIONS" title={t("reportsTitle")} description={t("reportsDesc")} actions={<EventSelect />} />
...
export function HistoryPage() {
  ...
  if (!selected) return <div className="page-stack"><PageHeader eyebrow="HISTORY" title={t("historyTitle")} description={t("historyDesc")} actions={<EventSelect includeHistory />} /><EmptyState icon={History} title={t("selectEventPrompt")} /></div>;
  return <div className="page-stack"><PageHeader eyebrow="HISTORY" title={t("historyTitle")} description={t("historyDesc")} actions={<EventSelect includeHistory />} />
...
export function MaterialsPage() {
  ...
  if (!event) return <div className="page-stack"><PageHeader eyebrow="MATERIAL DELIVERY" title={t("materialsTitle")} description={t("materialsDesc")} /><EmptyState icon={BookOpenCheck} title={t("empty")} /></div>;
  return <div className="page-stack"><PageHeader eyebrow="MATERIAL DELIVERY" title={t("materialsTitle")} description={t("materialsDesc")} actions={<label className="event-context-select"><span>{t("browseEvents")}</span><select value={event.id} onChange={(change) => selectActiveEvent(change.target.value)}>{materialEvents.map((item) => <option key={item.id} value={item.id}>{localize(item.title, language)}</option>)}</select></label>} />
...
export function SurveyPage() {
  ...
  if (!event) return <EmptyState icon={ClipboardClock} title={t("empty")} />;
  return <div className="page-stack"><PageHeader eyebrow="SURVEY DELIVERY" title={t("surveyTitle")} description={t("surveyDesc")} actions={<EventSelect module="survey" />} />
...
export function EarlyBirdPage() {
  ...
  if (!event) return <EmptyState icon={Gift} title={t("empty")} />;
  return <div className="page-stack"><PageHeader eyebrow="EARLY ARRIVAL" title={t("earlyTitle")} description={t("earlyDesc")} actions={<EventSelect module="earlyBird" />} />
...
export function LotteryPage() {
  ...
  if (!event) return <EmptyState icon={Sparkles} title={t("empty")} />;
  return <div className="page-stack lottery-page"><PageHeader eyebrow="EVENT LOTTERY" title={t("lotteryTitle")} description={t("lotteryDesc")} actions={<EventSelect module="lottery" />} />
